...
1.  **플러그인 설치**: 
    - 터미널을 열고 옵시디언 플러그인 폴더(`.obsidian/plugins/`)로 이동합니다.
    - 아래 명령어를 입력하여 저장소를 클론하면 모든 준비가 끝납니다 (`main.js`가 포함되어 있어 즉시 사용 가능):
      ```bash
      git clone https://github.com/wis-graph/obsidian-sharepage.git
      ```
    - (선택사항) 코드를 직접 수정하거나 직접 빌드하고 싶다면:
      ```bash
      cd obsidian-sharepage && npm install && npm run build
      ```
